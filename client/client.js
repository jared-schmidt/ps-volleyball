...
Template.hello.helpers({
    ...
    team1: function() {
        ...
        if (team.length === 1 && team[0].hasOwnProperty('team')){
            if (_.findWhere(team[0].team, {_id: Meteor.userId()})){
                Session.set('chapp-username', Meteor.user().profile.name); //you could set the user name on user login
                Session.set('chapp-docid','home'); //The room identifier. Iron router's before action can be a great spot to set this.
                console.log("Chat home team");
            } else {
    ...
    team2: function() {
        ...
        if (team.length === 1 && team[0].hasOwnProperty('team')){
            if (_.findWhere(team[0].team, {_id: Meteor.userId()})){
                Session.set('chapp-username', Meteor.user().profile.name); //you could set the user name on user login
                Session.set('chapp-docid','away'); //The room identifier. Iron router's before action can be a great spot to set this.
                console.log("Chat Away team");
            }else {
